...
function buildNav(members) {
  let globalNav;
  let nav = '<h2><a href="index.html">JS Doc Learning Example</a></h2>';
  const seen = {};
  const seenTutorials = {};
  ...
  if (members.globals.length) {
    ...
    if (!globalNav) {
      ...
      nav += `<h3>${linkto('global', 'JS Doc Learning')}</h3>`;
    } else {
      nav += `<h3>JS Doc Learning</h3><ul>${globalNav}</ul>`;
    }
  }
...
exports.publish = (taffyData, opts, tutorials) => {
  ...
  if (members.globals.length) {
    generate('Js Doc Learning', [{ kind: 'globalobj' }], globalUrl);
  }
